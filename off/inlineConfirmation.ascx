<%@ Control Language="C#" ClassName="inlineConfirmation" %>
...
<script runat="server">
    ...
    protected void btnYes_Click(object sender, EventArgs e)
    {
        ...
        {
            AB.Offs.ConfirmOff(hidOffId.Value);
        }
        if (hidCommandName.Value.Equals(btnNotConfirm.CommandName))
        {
            AB.Offs.NotConfirmOff(hidOffId.Value);
        }
        refreshPage();
    }
</script>
<asp:MultiView ID="mvw" runat="server" ActiveViewIndex="0">
    <asp:View ID="vwIdle" runat="server">
        <asp:Button ID="btnConfirm" runat="server" CommandName="1" 
            CssClass="FormButtom" onclick="btnConfirm_Click" Text="تأييد" />
        &nbsp;<asp:Button ID="btnNotConfirm" runat="server" CommandName="2" 
            CssClass="FormButtom" Text="عدم تائيد" onclick="btnNotConfirm_Click" />
    </asp:View>
